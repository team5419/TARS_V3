...
/**
 * The class used to build and execute all of our autos. Look into the constructors to see how to
 * use.
 */
public class Auto {
  ...
  // The goal of the autoState variable is to allow us to make a simple if statement in the Run()
  // based for running the autos based on the constructor used
  ...
  /**
   * A more general overload for if there is a command from somewhere else that you want to run
   * specifically, and i guess want the code to look cleaner. Don't see much use for this yet.
   *
  ...
  public Auto(Swerve swerve, String autoName) {
    ...
    List<PathPlannerTrajectory> pathGroup =
        PathPlanner.loadPathGroup(autoName, new PathConstraints(3.5, 3));

    compiledAuto = swerve.getAutoBuilder().fullAuto(pathGroup);
    compiledAuto =
        compiledAuto.beforeStarting(() -> start()).andThen(Commands.runOnce(() -> end()));
  ...
  public Auto(List<Point> i_points, Swerve i_swerve, String i_autoName) {
    ...
    // compiledAuto.beforeStarting(() -> AutoStart()).andThen(Commands.runOnce(() ->
    // AutoFinished()));
  }

  /**
   * The Run function for all autos. Once the auto constructor has been called with the right
   * details, call the Run function to stat the auto.
   *
