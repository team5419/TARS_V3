...
public class AutoUtil {
  /**
   * Calculates a spline from a list of points. Ideally we would not use this function for any new
   * autos, but rather to re-adapt old autos into something that can be used quick
   *
  ...
  public static PathPlannerTrajectory CalculatePathsFromPoints(List<Point> points) {
    ...
    // ? For this implementation specifically: we will be following the "on the fly" spline
    // generation
    // ? Docs here:
    // https://github.com/mjansen4857/pathplanner/wiki/PathPlannerLib:-Java-Usage#on-the-fly-generation
    // ? Custom use of the path can be found here:
    // https://docs.wpilib.org/en/stable/docs/software/advanced-controls/trajectories/holonomic.html
    // ? However, you shouldn't need to do that. For example, on the SwerveDrive class there is a
    // command builder for following a set path.
    ...
    for (Point point : points) {
      pointList.add(
          new PathPoint(
              ...
              point.hasVelocityOverride
                  ? point.startVelocity
                  : 0)); // position, heading(direction of travel), holonomic rotation, possible
      // non-zero starting velocity
    }

    return PathPlanner.generatePath(
        new PathConstraints(Constants.SwerveConstants.maxSpeed, 2), pointList);
  }
}
