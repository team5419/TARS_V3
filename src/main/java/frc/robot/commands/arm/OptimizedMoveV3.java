...
public class OptimizedMoveV3 extends CommandBase {
    ...
    public OptimizedMoveV3(OptimizedArm arm, ArmTargets target) {
        ...
        // velocityLogger = Shuffleboard.getTab("Tuning").add("Velocity Wrist", 0.0).withWidget(BuiltInWidgets.kGraph).getEntry();
        // cleLogger = Shuffleboard.getTab("Tuning").add("Closed Loop Error", 0.0).withWidget(BuiltInWidgets.kGraph).getEntry();
        // voltageLogger = Shuffleboard.getTab("Tuning").add("Voltage", 0.0).withWidget(BuiltInWidgets.kGraph).getEntry();
        // currentLogger = Shuffleboard.getTab("Tuning").add("Current", 0.0).withWidget(BuiltInWidgets.kGraph).getEntry();
    ...
    public void initialize() {
        ...
        ArmWaypoints[] tracePoints = GraphStator.tracePath(
        ...
        waypoints.add(new ArmState(OptimizedArm.ticksToDegreesBicep(target.bicepTarget), OptimizedArm.ticksToDegreesWrist(target.wristTarget)));
    ...
    private void configAndMoveTo (ArmState state) {
        ...
        arm.configMotionMagic(
            GraphStator.calculateNewMotionMagic(
                ...
                new Waypoint(OptimizedArm.degreesToTicksBicep(state.bicep), OptimizedArm.degreesToTicksWrist(state.wrist)), 
                arm, true
            )
        );
