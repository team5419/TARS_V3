...
public class AutoBalance extends CommandBase {
  ...
  // GenericEntry rollLogger = Shuffleboard.getTab("Auto Balance").add("Roll",
  // 0.0).withWidget(BuiltInWidgets.kGraph).getEntry();
  // GenericEntry rollLoggerCalculated = Shuffleboard.getTab("Auto Balance").add("Filtered Roll",
  // 0.0).withWidget(BuiltInWidgets.kGraph).getEntry();
  // GenericEntry rollState = Shuffleboard.getTab("Auto Balance").add("State",
  // State.GENERAL.toString()).withWidget(BuiltInWidgets.kTextView).getEntry();

  // GenericEntry thresholdEntry = Shuffleboard.getTab("Auto Balance").add("Threshold",
  // 0.0).withWidget(BuiltInWidgets.kNumberSlider).getEntry();
  // GenericEntry angleEpsilonEntry = Shuffleboard.getTab("Auto Balance").add("Angle Epsilon",
  // 0.0).withWidget(BuiltInWidgets.kNumberSlider).getEntry();
  ...
  public boolean isFinished() {
    // return false;
    return MathUtil.applyDeadband(gyro.getRoll(), angleEpsilon) == 0
        && currentState == State.CORRECTION;
  }
}
