...
public class OptimizedArm extends SubsystemBase {
  ...
  // Get a tab and a toggle so that we don't have to re-deploy or power-cycle our bot to tune our
  // arm setpoints
  private ShuffleboardTab tab = Shuffleboard.getTab("Optimized Arm");
  private GenericEntry tuningModeToggle =
      tab.add("Tuning Mode", false).withWidget(BuiltInWidgets.kToggleButton).getEntry();
  private boolean isAtComp =
      DriverStation.isFMSAttached(); // So we only run this if we are not at competition

  public boolean isTesting = false;

  private Notifier updateDSInputs =
      new Notifier(
          () -> {
  ...
  static class MotionMagicConfig {
    ...
    public MotionMagicConfig(
        boolean isBicep, double cruiseVelocity, double acceleration, int sCurveStrength) {
  ...
  private MotionMagicConfig bicepBaseConfig =
      new MotionMagicConfig(
          true,
          Constants.ArmConstants.BASE_MAX_V,
          Constants.ArmConstants.BASE_MAX_A,
          Constants.ArmConstants.BASE_CURVE_STR);
  private MotionMagicConfig wristBaseConfig =
      new MotionMagicConfig(
          false,
          Constants.ArmConstants.WRIST_MAX_V,
          Constants.ArmConstants.WRIST_MAX_A,
          Constants.ArmConstants.WRIST_CURVE_STR);

  private MotionMagicConfig bicepSlowConfig =
      new MotionMagicConfig(
          true,
          Constants.ArmConstants.BASE_MAX_V / 10,
          Constants.ArmConstants.BASE_MAX_A / 10,
          Constants.ArmConstants.BASE_CURVE_STR);
  private MotionMagicConfig wristSlowConfig =
      new MotionMagicConfig(
          false,
          Constants.ArmConstants.WRIST_MAX_V / 10,
          Constants.ArmConstants.WRIST_MAX_A / 10,
          Constants.ArmConstants.WRIST_CURVE_STR);
  ...
  public OptimizedArm(boolean isTesting) {
    ...
    if (isTesting) {
      ...
      bicepTalonFollower.configSupplyCurrentLimit(
          new SupplyCurrentLimitConfiguration(true, 10, 20, 0.1));
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber(
        "Wrist - CURRENT DEG",
        OptimizedArm.ticksToDegreesWrist(wristTalon.getSelectedSensorPosition()));
    SmartDashboard.putNumber(
        "Wrist - SETPOINT DEG", OptimizedArm.ticksToDegreesWrist(wristTalon.getClosedLoopTarget()));
  ...
  public boolean isAtWithEpsilon(ArmState waypoint, double epsilon) {
    return (Math.abs(getBicepPosition() - degreesToTicksBicep(waypoint.bicep)) < epsilon)
        && (Math.abs(getWristPosition() - degreesToTicksWrist(waypoint.wrist)) < epsilon);
  }

  public boolean isAtWithEpsilon(ArmTargets target, double epsilon) {
    return (Math.abs(getBicepPosition() - target.bicepTarget) < epsilon)
        && (Math.abs(getWristPosition() - target.wristTarget) < epsilon);
  }

  public boolean bicepAtTarget(double target) {
    return (Math.abs(bicepTalon.getSelectedSensorPosition() - target)
        < Constants.ArmConstants.BASE_ERROR_THRESHOLD);
  }

  public boolean wristAtTarget(double target) {
    return (Math.abs(wristTalon.getSelectedSensorPosition() - target)
        < Constants.ArmConstants.BASE_ERROR_THRESHOLD);
  }
