...
public enum GraphStator {
  /** Arranged like this E B A D F C */
  ...
  GraphStator(
      LegalState legalState, ArmState min, ArmState max, ArmWaypoints[][] intermediatePath) {
    this.min = min;
    this.max = max;
  ...
  public static ArmWaypoints[] tracePath(ArmState startWaypoint, ArmTargets targetWaypoint) {
    ...
    GraphStator end =
        getSectorStateFromCoords(
            OptimizedArm.ticksToDegreesBicep(targetWaypoint.bicepTarget),
            OptimizedArm.ticksToDegreesWrist(targetWaypoint.wristTarget));
   ...
   */
  public static MotionMagicConfig[] calculateNewMotionMagic(
      Waypoint start, Waypoint end, OptimizedArm arm, boolean isLastMove) {
    double bicepDiff =
        Conversions.degreesToFalcon(
            Math.abs(start.bicep - end.bicep), Constants.ArmConstants.BASE_GEAR_RATIO);
    double wristDiff =
        Conversions.degreesToFalcon(
            Math.abs(start.wrist - end.wrist), Constants.ArmConstants.WRIST_GEAR_RATIO);
    ...
    if (ratio < 0.0001) { // Prevent incredibly small ratios
      return new MotionMagicConfig[] {
        arm.getBaseConfig(true), arm.getBaseConfig(false)
      }; // change nothing, but have to return something
    }
    ...
    if (ratio < 1) {
      return new MotionMagicConfig[] {
        new MotionMagicConfig(
            true,
            Constants.ArmConstants.BASE_MAX_V * ratio * slowDownEvenMore,
            Constants.ArmConstants.BASE_MAX_A,
            3),
        new MotionMagicConfig(
            false,
            Constants.ArmConstants.WRIST_MAX_V * slowDownEvenMore,
      ...
      return new MotionMagicConfig[] {
        new MotionMagicConfig(
            false,
            Constants.ArmConstants.WRIST_MAX_V / ratio * slowDownEvenMore,
            Constants.ArmConstants.WRIST_MAX_A,
            3),
        new MotionMagicConfig(
            true,
            Constants.ArmConstants.BASE_MAX_V * slowDownEvenMore,
            Constants.ArmConstants.BASE_MAX_A,
            3)
      };
    } else {
